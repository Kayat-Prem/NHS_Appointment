...
export default function Landing() {
  ...
  const navItems = [
    ...
    },
    {
      ...
      dropdown: [
        ...
        { label: 'Prescriptions',   onClick: () => navigate('/precriptions') },
      ]
    },
    {
      label: 'How it works',
    ...
    },
    {
      label: 'Health Info',
      dropdown: [
        { label: 'NHS services',      onClick: () => navigate('/NhsService') },
        { label: 'When to see a GP',  onClick: () => navigate('/when-to-see-gp') },
        { label: 'Common Conditions',       onClick: () => navigate('/common-conditions') },
        { label: 'Mental Health',       onClick: () => navigate('/mental-health') },
        { label: 'Healthy Living',       onClick: () => navigate('/healthy-living') },
        { label: 'Accessibility',     onClick: () => navigate('/nhs-policy') },
        { label: 'Privacy & Data',       onClick: () => navigate('/privacy') },
    ...
    },
    {
      ...
      dropdown: [
        ...
        { label: 'Cookie Policy',  onClick: () => navigate('/cookie') },
      ]
    },
  ...
  const S = {
    ...
    sectionLabel: {
      fontSize: '13px', fontWeight: '700', color: '#005EB8',
      letterSpacing: '1.5px', marginBottom: '12px', textTransform: 'uppercase'
    },
    sectionTitle: {
      fontSize: '32px', fontWeight: '700', color: '#212b32',
      marginBottom: '12px', lineHeight: 1.3
    },
    sectionSub: {
      fontSize: '18px', color: '#4c6272',
      marginBottom: '44px', lineHeight: 1.6
    },
  }

  const footerLinkStyle = (isHovered) => ({
    fontSize: '14px',
    ...
    <div style={S.page}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#fff', borderRadius: '4px', overflow: 'hidden', height: '36px' }}>
              <input
                type="text"
                placeholder="Search..."
                style={{ border: 'none', outline: 'none', padding: '0 14px', fontSize: '14px', width: '200px', height: '100%', fontFamily: 'inherit', color: '#212b32' }}
              />
            ...
            <button
              onClick={() => navigate('/login')}
              style={{ padding: '8px 20px', background: 'transparent', border: '1.5px solid rgba(255,255,255,0.6)', color: '#fff', borderRadius: '4px', fontSize: '14px', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Sign in
            </button>
            <button
              onClick={() => navigate('/register')}
              style={{ padding: '8px 20px', background: '#007f3b', border: 'none', color: '#fff', borderRadius: '4px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Register
            </button>
          </div>
            ...
            {navItems.map((item) => (
              ...
              >
                <div
                  ...
                  style={{
                    padding: '15px 18px', color: '#fff', fontSize: '15px',
                    fontWeight: item.highlight ? '700' : '500', cursor: 'pointer',
                    display: 'flex', alignItems: 'center', gap: '5px',
                    borderBottom: activeDropdown === item.label ? '3px solid #fff' : '3px solid transparent',
                    background: item.highlight ? '#007f3b' : activeDropdown === item.label ? 'rgba(255,255,255,0.1)' : 'transparent',
                ...
                >
                  ...
                  {item.dropdown && (
                    <i
                      className={`ti ti-chevron-${activeDropdown === item.label ? 'up' : 'down'}`}
                ...
                {item.dropdown && activeDropdown === item.label && (
                  <div style={{
                    ...
                    minWidth: '230px', zIndex: 2000,
                    borderTop: '3px solid #005EB8', overflow: 'hidden'
                  }}>
                    {item.dropdown.map((drop, di) => (
                      <div
                        ...
                        style={{
                          padding: '13px 20px', fontSize: '15px', color: '#212b32',
                          cursor: 'pointer',
                          borderBottom: di < item.dropdown.length - 1 ? '0.5px solid #f0f4f5' : 'none',
      ...
      <div style={{ width: '100%', background: '#005EB8', padding: '90px 6%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '52px', flexWrap: 'wrap', boxSizing: 'border-box' }}>
        <div style={{ flex: 1, minWidth: '320px' }}>
          <span style={{ display: 'inline-block', background: 'rgba(255,255,255,0.18)', color: '#fff', fontSize: '13px', fontWeight: '700', padding: '6px 16px', borderRadius: '20px', marginBottom: '22px', letterSpacing: '1px' }}>
            NHS DIGITAL SERVICE
          </span>
          ...
          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
            <button
              onClick={() => navigate('/register')}
              style={{ padding: '16px 36px', background: '#007f3b', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '17px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Book an appointment
            </button>
            <button
              onClick={() => navigate('/login')}
              style={{ padding: '16px 36px', background: 'transparent', color: '#fff', border: '2px solid rgba(255,255,255,0.75)', borderRadius: '4px', fontSize: '17px', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Sign in
            </button>
          </div>
        ...
        <div style={{ flex: '0 0 420px', minWidth: '300px' }}>
          <div style={{ background: '#fff', borderRadius: '12px', padding: '36px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                { icon: 'ti-calendar-plus', bg: '#E1F5EE', color: '#007f3b', label: 'Book appointment', sub: 'Choose a GP and time slot', path: '/register' },
                { icon: 'ti-calendar',      bg: '#E6F1FB', color: '#005EB8', label: 'My appointments',  sub: 'View and manage bookings',  path: '/login' },
                { icon: 'ti-user',          bg: '#FAEEDA', color: '#633806', label: 'My profile',        sub: 'Update your details',       path: '/login' },
              ].map(link => (
                <div
                  key={link.label}
                  onClick={() => navigate(link.path)}
                  style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 18px', background: '#f0f4f5', borderRadius: '8px', cursor: 'pointer', border: '0.5px solid #d8dde0' }}
                >
          ...
          {roles.map(role => (
            <div key={role.title} style={{ background: '#fff', borderRadius: '12px', padding: '32px', border: '0.5px solid #d8dde0', borderTop: `4px solid ${role.topColor}` }}>
              ...
              <button
                ...
                style={{ width: '100%', padding: '14px', background: role.btnBg, color: '#fff', border: 'none', borderRadius: '6px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}
              >
                {role.btnLabel}
              </button>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px' }}>
          {steps.map((step) => (
            <div key={step.num} style={{ textAlign: 'center', padding: '32px 24px', background: '#f0f4f5', borderRadius: '12px', border: '0.5px solid #d8dde0', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', top: '20px', right: '20px', width: '28px', height: '28px', borderRadius: '50%', background: '#E6F1FB', color: '#005EB8', fontSize: '13px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {step.num}
              </div>
      ...
      <div style={{ width: '100%', background: '#005EB8', padding: '90px 6%', textAlign: 'center', boxSizing: 'border-box' }}>
        <div style={{ fontSize: '36px', fontWeight: '700', color: '#fff', marginBottom: '14px' }}>
          Ready to book your appointment?
        </div>
        <div style={{ fontSize: '19px', color: 'rgba(255,255,255,0.85)', marginBottom: '36px' }}>
          Join thousands of patients already using the NHS online booking service
        </div>
      ...
      <div style={{ width: '100%', background: '#212b32', boxSizing: 'border-box' }}>
        ...
        <div style={{ padding: '56px 6%', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '40px', boxSizing: 'border-box' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[
                ...
                <a key={icon} href={url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <div
                    style={{
          ...
          {/* Our Services */}
          <div>
            <div style={{ fontSize: '15px', fontWeight: '700', color: '#fff', marginBottom: '16px', paddingBottom: '10px', borderBottom: '1px solid rgba(255,255,255,0.15)' }}>
              Our Services
            </div>
            {[
              { label: 'Book an appointment',  path: '/register' },
              { label: 'Find a GP near you',   path: '/register' },
              { label: 'View my appointments', path: '/login' },
              { label: 'Cancel appointment',   path: '/login' },
              { label: 'Patient portal',       path: '/login' },
              { label: 'GP portal',            path: '/login' },
            ].map(item => (
              <div
                key={item.label}
                onClick={() => navigate(item.path)}
                style={{ fontSize: '14px', color: 'rgba(255,255,255,0.65)', marginBottom: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}
          ...
          <div>
            ...
            {[
              { icon: 'ti-phone',   text: '0800 123 4567',        sub: 'Mon–Fri, 8am–6pm',       path: '/contact' },
              { icon: 'ti-mail',    text: 'support@nhsgp.nhs.uk', sub: 'Reply within 24 hours',  path: '/contact' },
              { icon: 'ti-map-pin', text: 'NHS Digital, Leeds',   sub: 'LS2 7UE, United Kingdom', path: '/contact' },
              { icon: 'ti-urgent',  text: 'Medical Emergency?',   sub: 'Call 999 immediately',    path: null },
            ].map(item => (
              <div
                key={item.text}
                onClick={() => item.path && navigate(item.path)}
                style={{ display: 'flex', gap: '12px', marginBottom: '16px', alignItems: 'flex-start', cursor: item.path ? 'pointer' : 'default' }}
                onMouseEnter={e => { if (item.path) e.currentTarget.style.opacity = '0.8' }}
                onMouseLeave={e => { e.currentTarget.style.opacity = '1' }}
              >
                <div style={{ width: '32px', height: '32px', borderRadius: '6px', background: 'rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div>
            ...
            {[
              { label: 'About this service',      path: '/about' },
              { label: 'Privacy policy',          path: '/privacy' },
              { label: 'NHS digital policy',      path: '/nhs-policy' },
        ...
        <div style={{ background: '#1a2028', padding: '18px 6%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', boxSizing: 'border-box' }}>
          <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)' }}>
            © 2026 NHS GP Appointment Booking Service. All rights reserved.
          </span>
          <div style={{ display: 'flex', gap: '20px' }}>
            {[
              { label: 'Privacy',       path: '/privacy' },
              { label: 'Cookie Policy', path: '/cookie' },
              { label: 'Accessibility', path: '/nhs-policy' },
