...
export default function GPLayout({ children }) {
  ...
  const navItems = [
    { path: '/gp/dashboard', label: 'Dashboard',     icon: 'ti-layout-dashboard' },
    { path: '/gp/slots',     label: 'Manage slots',  icon: 'ti-clock' },
    { path: '/gp/schedule',  label: 'My schedule',   icon: 'ti-calendar' },
    { path: '/gp/patients',  label: 'My patients',   icon: 'ti-users' },
  ]
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f5', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#005EB8', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '52px', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <span style={{ color: 'rgba(255,255,255,0.85)', fontSize: '13px' }}>Dr. {user?.name}</span>
